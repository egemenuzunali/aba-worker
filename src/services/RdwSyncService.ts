...
export class RdwSyncService {
	...
	private async checkApkExpiryForCompany(companyId: string): Promise<number> {
		...
		for (let skip = 0; skip < totalClients; skip += CLIENT_BATCH_SIZE) {
			...
			// Use $limit in the $group stage to prevent memory issues
			const result = await db.default.models.Vehicle.aggregate([
				...
				},
				{
					...
					$project: {
						_id: 1,
						count: 1,
						// Only keep first N vehicle IDs, but preserve the accurate count
		...
		if (totalExpiredCount > 0) {
			await NotificationService.createApkExpiredNotification(
				companyId,
				totalExpiredCount, // Use accurate count
				expiredVehicleIds   // Limited vehicle IDs for metadata
			);
		...
		if (totalExpiringCount > 0) {
			await NotificationService.createApkExpiringNotification(
				companyId,
				totalExpiringCount, // Use accurate count
				expiringVehicleIds  // Limited vehicle IDs for metadata
			);
			notificationsCreated++;
